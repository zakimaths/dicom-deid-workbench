...
| NIfTI volume viewer | Curated teaching volumes and a prepared defacing comparison publicly; locally, scalar 3D NIfTI-1 .nii/.nii.gz, uint8/int16/uint16/float32 | Three slice planes; locally, header-cleaned .nii with reopened-output checks | 32 MiB input, 64 MiB unpacked, 16,777,216 voxels, coded orientation; header cleaning does not remove anatomy, labels or sidecars; no BIDS or 4D; [full profile](nifti.md) |
...
Native X-ray DICOM, compression, enhanced multiframe, colour, LUT/padding workflows are unsupported. The teaching library's X-ray pictures are PNG/JPEG views, not native X-ray DICOM processing.
...
Experimental [mask-guided NIfTI removal](nifti-defacing.md) accepts a narrower subset: axis-aligned non-negative unscaled structural 3D MRI with an independently reviewed binary brain mask on exactly the same grid. It does not generate segmentation or certify anonymity.

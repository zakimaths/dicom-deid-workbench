...
| R05: no stale edit result | server.py current job, invalidation and TTL | test_redaction_replaces_job_and_rejects_stale_export; checked-in delayed-download browser regressions | Pending UI edits are not a server-side review record |
...
The [0.2.1 adversarial audit](audit-0.2.1.md) adds explicit regression evidence and cross-browser automation; the clinical and formal-conformance limits above remain.

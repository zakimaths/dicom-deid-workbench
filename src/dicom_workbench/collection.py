...
def transform_collection(files):
    if not isinstance(files, (list, tuple)) or not 1 <= len(files) <= MAX_FILES:
        raise Unsupported("Choose between 1 and 512 supported files from one study.")
    ...
    if sum(map(len, files)) > MAX_COLLECTION_BYTES:
        raise Unsupported("A collection must total at most 128 MiB.")
    ...
    if any(
        any(e.VR == "SQ" and e.keyword.startswith("Referenced") for e in ds.iterall())
        for ds in datasets
    ):

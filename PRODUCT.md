...
The public `/nifti.html` page offers a CC0 downsampled structural brain MRI and a deterministic asymmetric block volume. The local `/nifti` page additionally inspects supported scalar 3D NIfTI-1 files and creates a new header-cleaned `.nii`. The allowed profile, byte/voxel limits, provenance and tests are in `docs/nifti.md`. Slice direction, step navigation, contrast, fit and 50–300% zoom are ordinary labelled controls. Numeric geometry, scaling and voxel data are checked after serialisation; text fields, extensions and unused bytes are removed. Facial anatomy, labels and accompanying files remain unassessed. The local workspace also offers experimental mask-guided whole-volume removal with linked before/after and mask views, a 2–20 mm protection margin, per-voxel verification and review-gated downloads. It requires a separately reviewed binary brain mask, axis-aligned non-negative unscaled structural MRI and matching grids. Public Pages offers only a prepared MNI average-head comparison. No automatic segmentation, BIDS or anonymity certification is claimed. Details and template limitations are in `docs/nifti-defacing.md`.
